...
class Fireball {
  ...
  spawn(direction) {
    ...
    element.position = {
      x: Number(playerElement.style.left.replace('px', '')) + 30 * player.directon,
      y: Number(playerElement.style.top.replace('px', '')) + 10,
    }
    ...
    setInterval(() => {
      element.position.x =
        direction == 1
          ? Number((element.position.x += this.chageSpeed))
          : Number((element.position.x -= this.chageSpeed))
